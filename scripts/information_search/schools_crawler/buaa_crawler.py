...
class BUAACrawler:
    ...
    def __init__(self):
        self.base_url = 'https://lqcx.buaa.edu.cn'
        self.headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            'Referer': 'https://lqcx.buaa.edu.cn/static/front/buaa/basic/html_web/lnfs.html',
            'X-Requested-With': 'XMLHttpRequest'
        }
        self.session = requests.Session()
        self.session.headers.update(self.headers)
        self.school_name = '北京航空航天大学'
        self.school_code = '10006'  # 北京航空航天大学招生代码
    ...
    def crawl_by_year_and_province(self, year: int, province: str = '', klmc: str = '', zslx: str = '') -> List[Dict]:
        ...
        params = {
            ...
            'klmc': klmc if klmc else '',  # 科类名称，不能为空
            'sex': '',  # 性别
            'campus': '',  # 校区
            'zslx': zslx if zslx else ''  # 招生类型，不能为空
        }
    ...
    def get_available_klmc_zslx_for_province_year(self, province: str, year: int) -> List[tuple]:
        """
        获取指定省份和年份的可用科类和招生类型组合列表
        ...
        for item in filter_list:
            if isinstance(item, dict):
                # 键格式：省份_年份_科类_sex_campus
                # 值：招生类型列表
                for key, zslx_list in item.items():
                    parts = key.split('_')
                    if len(parts) >= 3:
                        item_province = parts[0]
                        item_year = parts[1]
                        item_klmc = parts[2]
                        
                        # 检查是否匹配省份和年份
                        if item_province == province and str(item_year) == str(year):
                            # 遍历招生类型列表
                            if isinstance(zslx_list, list):
                                for zslx in zslx_list:
                                    if zslx:  # 确保招生类型不为空
                                        combinations.append((item_klmc, zslx))
                            elif zslx_list:  # 如果是单个值
                                combinations.append((item_klmc, zslx_list))
    ...
    def crawl_by_year(self, year: int, provinces: List[str] = None) -> List[Dict]:
                ...
                for item in filter_list:
                    if isinstance(item, dict):
                        for key in item.keys():
                            parts = key.split('_')
                            if len(parts) >= 1:
                                available_provinces.add(parts[0])
            
            available_provinces = list(available_provinces)
        ...
        # 逐个省份爬取，每个省份需要获取其可用科类和招生类型组合
        for province in available_provinces:
            if province:  # 确保省份名称不为空
                # 获取该省份该年份的可用科类和招生类型组合
                combinations = self.get_available_klmc_zslx_for_province_year(province, year)
                
                if combinations:
                    # 逐个组合爬取
                    for klmc, zslx in combinations:
                        data = self.crawl_by_year_and_province(year, province, klmc, zslx)
                        all_data.extend(data)
                else:
                    # 如果没有找到组合，记录警告
                    logger.warning(f"未找到 {province} {year}年的可用科类和招生类型组合")
